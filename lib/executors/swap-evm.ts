 ...
 *
 * Executes EVM swaps using Paraswap API — no API key required.
...
export class EvmSwapExecutor {
  ...
  constructor(
    ...
    this.provider = new ethers.JsonRpcProvider(
      CHAIN_RPC[this.chain] ?? CHAIN_RPC.arbitrum,
    );
  ...
  async execute(): Promise<Record<string, unknown>> {
    ...
    const slippagePct = parseFloat(String(slippage ?? "0.5")); // Paraswap takes %
    ...
    const rawAmount = ethers
      .parseUnits(amtNum.toFixed(Math.min(dec, 8)), dec)
      .toString();

    console.log(
      `🔄 [EVM/${this.chain}] ${amtNum} ${fromUpper} → ${toUpper} via Paraswap`,
    );
    console.log(`👛 Wallet: ${this.wallet.address}`);

    try {
      // ── Step 1: Get price/route from Paraswap ──────────────────────────────
      const priceUrl = new URL(`${PARASWAP}/prices`);
      priceUrl.searchParams.set("srcToken", srcAddr);
      priceUrl.searchParams.set("destToken", dstAddr);
      priceUrl.searchParams.set("amount", rawAmount);
      priceUrl.searchParams.set("srcDecimals", String(dec));
      priceUrl.searchParams.set(
        "destDecimals",
        String(TOKEN_DECIMALS[toUpper] ?? 18),
      );
      ...
      if (!priceRes.ok) {
        ...
        throw new Error(`Paraswap price error: ${txt}`);
      }
      const priceData = await priceRes.json();
      const priceRoute = priceData.priceRoute;
      if (!priceRoute) throw new Error("Paraswap returned no route");

      console.log(
        `[Paraswap] route: ${priceRoute.bestRoute?.[0]?.swaps?.[0]?.swapExchanges?.[0]?.exchange ?? "auto"}`,
      );

      // ── Step 2: Build the transaction ──────────────────────────────────────
      const txRes = await fetch(
        `${PARASWAP}/transactions/${this.chainId}?ignoreChecks=true`,
        {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            slippage: Math.round(slippagePct * 100), // Paraswap takes bps here
      ...
      if (!txRes.ok) {
        ...
        throw new Error(`Paraswap tx build error: ${txt}`);
      }
      ...
      if (srcAddr !== NATIVE) {
        const spender = tx.to; // Paraswap's TokenTransferProxy
        const token = new ethers.Contract(srcAddr, ERC20_ABI, this.wallet);
        const allowance: bigint = await token.allowance(
          this.wallet.address,
          spender,
        );
        if (allowance < BigInt(rawAmount)) {
          console.log(`[Paraswap] approving spender ${spender}`);
          const approveTx = await token.approve(spender, ethers.MaxUint256);
          await approveTx.wait(1);
          console.log(`[Paraswap] approval confirmed`);
        }
      }
      ...
      console.log(`[Paraswap] tx sent: ${txResponse.hash}`);
