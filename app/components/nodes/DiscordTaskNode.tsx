...
export const DiscordTaskNode = memo(({ data, selected, id }: NodeProps) => {
  ...
  const serverId = String(data.serverId ?? "discord.gg/...");

  const updateNodeData = useFlowStore((state) => state.updateNodeData);

  const customColor = data.customColor as string | undefined;
  const defaultColor = "#818cf8"; // indigo-400
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm">Discord Task</span>
        </div>
      </div>
      <div className="bg-slate-900/95 backdrop-blur-xl p-3 border-t border-white/5 select-none rounded-b-xl">
        <div className="space-y-1">
          ...
          <div className="text-[10px] text-slate-400 truncate">{serverId}</div>
        </div>
      </div>
      <Handle
        type="target"
