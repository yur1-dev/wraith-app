...
export const VolumeFarmerNode = memo(({ data, selected, id }: NodeProps) => {
  const swapCount = String(data.swapCount ?? 5);
  const swapAmount = String(data.swapAmount ?? 5);
  const targetVolume = String(data.targetVolume ?? 100);
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm">Volume Farmer</span>
        </div>
      </div>
