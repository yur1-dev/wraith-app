...
export const MultiWalletNode = memo(({ data, selected, id }: NodeProps) => {
  ...
  const executeSequentially = Boolean(data.executeSequentially);

  const updateNodeData = useFlowStore((state) => state.updateNodeData);

  const customColor = data.customColor as string | undefined;
  const defaultColor = "#f97316"; // orange-500
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm">Multi-Wallet</span>
        </div>
      </div>
      ...
      <div className="bg-slate-900/95 backdrop-blur-xl p-3 border-t border-white/5 select-none rounded-b-xl">
        <div className="space-y-1">
          ...
          <div className="text-[10px] text-slate-400">
            {executeSequentially ? "Sequential" : "Parallel"} execution
          </div>
        </div>
      </div>
