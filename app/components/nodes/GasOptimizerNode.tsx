...
import { Fuel, MoreVertical } from "lucide-react";
import { useFlowStore } from "@/lib/hooks/useFlowStore";

export const GasOptimizerNode = memo(({ data, selected, id }: NodeProps) => {
  const maxGas = String(data.maxGas ?? 20);
  const timeout = String(data.timeout ?? 60);

  const updateNodeData = useFlowStore((state) => state.updateNodeData);

  const customColor = data.customColor as string | undefined;
  const defaultColor = "#84cc16"; // lime-400
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm">Gas Optimizer</span>
        </div>
      </div>
