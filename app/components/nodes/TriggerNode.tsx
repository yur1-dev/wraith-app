...
export const TriggerNode = memo(({ data, selected, id }: NodeProps) => {
  const scheduleType = String(data.scheduleType ?? "Daily");
  const scheduleTime = String(data.scheduleTime ?? "3:00 AM");
  const cronExpression =
    data.cronExpression != null ? String(data.cronExpression) : null;
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm">Schedule Trigger</span>
        </div>
      </div>
      ...
      <div className="bg-slate-900/95 backdrop-blur-xl p-3 border-t border-white/5 select-none rounded-b-xl">
        <div className="space-y-1">
          <div className="text-xs font-medium" style={{ color: accentColor }}>
            {scheduleType} at {scheduleTime}
          </div>
          {cronExpression && (
            <div
              className="text-xs font-mono bg-slate-800/80 px-2 py-1 rounded border"
