...
export const ClaimAirdropNode = memo(({ data, selected, id }: NodeProps) => {
  const projectName = String(data.projectName ?? "ProjectXYZ");
  const contractAddress = String(data.contractAddress ?? "0x...");

  const updateNodeData = useFlowStore((state) => state.updateNodeData);

  const customColor = data.customColor as string | undefined;
  const defaultColor = "#f43f5e"; // rose-500
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm">Claim Airdrop</span>
        </div>
      </div>
