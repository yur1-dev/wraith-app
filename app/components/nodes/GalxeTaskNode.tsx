...
export const GalxeTaskNode = memo(({ data, selected, id }: NodeProps) => {
  const campaignName = String(data.campaignName ?? "Project Campaign");
  const campaignUrl = String(data.campaignUrl ?? "galxe.com/...");

  const updateNodeData = useFlowStore((state) => state.updateNodeData);

  const customColor = data.customColor as string | undefined;
  const defaultColor = "#a78bfa"; // violet-400
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm">Galxe Task</span>
        </div>
      </div>
