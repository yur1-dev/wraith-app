...
export const SwapNode = memo(({ data, selected, id }: NodeProps) => {
  ...
  const fromToken = String(data.fromToken ?? "TOKEN");
  const toToken = String(data.toToken ?? "TOKEN");
  const slippage = String(data.slippage ?? 1);
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm">Swap</span>
        </div>
      </div>
