...
export const LendStakeNode = memo(({ data, selected, id }: NodeProps) => {
  const actionType = String(data.actionType ?? "stake");
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm">Lend/Stake</span>
        </div>
      </div>
      ...
      <div className="bg-slate-900/95 backdrop-blur-xl p-3 border-t border-white/5 select-none rounded-b-xl">
        <div className="space-y-1">
          <div className="text-xs font-medium" style={{ color: accentColor }}>
            {actionType === "lend" ? "Lend" : "Stake"} {amount} {token}
          </div>
          <div className="text-[10px] text-slate-400">on {protocol}</div>
        </div>
      </div>
