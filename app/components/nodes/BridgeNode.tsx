...
export const BridgeNode = memo(({ data, selected, id }: NodeProps) => {
  const fromChain = String(data.fromChain ?? "Ethereum");
  const toChain = String(data.toChain ?? "Arbitrum");
  const bridgeProtocol = String(data.bridgeProtocol ?? "LayerZero");
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm">Bridge</span>
        </div>
      </div>
      <div className="bg-slate-900/95 backdrop-blur-xl p-3 border-t border-white/5 select-none rounded-b-xl">
        <div className="space-y-1">
          ...
          <div className="text-[10px] text-slate-400">via {bridgeProtocol}</div>
        </div>
      </div>
      <Handle
        type="target"
